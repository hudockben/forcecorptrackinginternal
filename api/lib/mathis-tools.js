...
function toolsFor(scope) {
  ...
  tools.push({
    ...
    description:
      "Fetch the asking user's OWN timesheet entries for the last 45 days — hours logged, what is still in draft, what has been approved. Nobody else's entries are available through this or any other tool. This data carries no pay rate, so it cannot answer anything about money.",
...
function stepLabel(name, input) {
  if (name === 'get_my_timesheet') return 'Reading your timesheet';
...
async function runTool(c, name, input) {
  if (name === 'get_my_timesheet') {
    return { digest: await digests.personalDigest(c) };
  }
