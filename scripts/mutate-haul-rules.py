...
MUT=[
 ...
 ("per-row: a priced machine is confused with the named truck again","payroll.html",
  "        if (!splitPricedMachineOnRow(r) || splitTruckOnRow(r)) return;",
  "        if (!splitPricedMachineOnRow(r)) return;","scripts/test-haul-unpriced-warning.js"),
 ...
 ("per-row: the stored answer is dropped on insert","api/timesheet-entries.js",
  "        ${r.is_haul === true || r.is_haul === false ? r.is_haul : null}",
  "        ${null}","scripts/test-haul-row-rate.js"),
 ("sweep: ignore the stored answer and re-haul the row","api/timesheet-entries.js",
  "  if (row && row.is_haul === false) return { is_haul: false };","",
  "scripts/test-haul-unpriced-warning.js"),
 ("stored answer: let a stale stamp outrank the column","api/timesheet-entries.js",
  "  if (row && row.is_haul === true)  return { is_haul: true };\n  if (row && row.is_haul === false) return { is_haul: false };\n  if (HAUL_FIELD_TYPE_RE",
  "  if (HAUL_FIELD_TYPE_RE",
  "scripts/test-haul-unpriced-warning.js"),
 ("stored answer: forget the rows approved before the column existed","api/timesheet-entries.js",
  "  if (HAUL_FIELD_TYPE_RE.test(String((row && row.field_type) || ''))) return { is_haul: true };\n  return null;",
  "  return null;","scripts/test-haul-row-rate.js"),
 ...
 ("sweep: clear haul_hours instead of adjusting it","api/timesheet-entries.js",
  ...
  "                m.work, NULL","scripts/test-haul-row-rate.js"),
 ("edit: clear haul_hours on an edit that kept the answer","api/timesheet-entries.js",
  "          haul_hours         = CASE WHEN ${keepHaul}::boolean\n                                    THEN haul_hours ELSE NULL END,",
  "          haul_hours         = NULL,","scripts/test-haul-row-rate.js"),
 ("bulk: stop sending the driver's answer and let the truck decide","payroll.html",
  "        r.is_haul = true;","","scripts/test-bulk-equipment-overrides.js"),
 ("bulk: stop naming the days it cannot answer for","payroll.html",
  "      const review = bulkHaulNeedsReview(g);","      const review = [];",
  "scripts/test-bulk-equipment-overrides.js"),
